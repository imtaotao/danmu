...
<Tabs>
  <TabItem label="npm">```sh npm install danmu ```</TabItem>
  <TabItem label="pnpm">```sh pnpm install danmu ```</TabItem>
  <TabItem label="Yarn">```sh yarn add danmu ```</TabItem>
</Tabs>
...
const manager = create({
  ...
  plugin: {
    ...
    willRender(data) {
      ...
      DataTransfer.prevent = true; // 设置为 true 将阻止渲染，可以在这里做弹幕过滤工作
      return data;
    },
  },
})
