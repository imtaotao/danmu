...
```tsx
// 点击弹幕的时候会销毁弹幕
function Danmaku({ danmaku }: { danmaku: Danmaku<unknown> }) {
  return <div onClick={() => danmaku.destory()}>{danmaku.data.value}</div>;
}
...
`getHeight` 将返回弹幕实例自身的宽度，同样的在你发送高级弹幕的时候会很有用。
