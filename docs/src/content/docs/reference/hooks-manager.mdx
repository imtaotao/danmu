...
钩子分为全局的 `manager` 钩子和 `弹幕` 的钩子，`T` 是弹幕内容类型，由用户创建 `manager` 时传入。
...
- **`$hide: SyncHook<Danmaku<T>>`**
- **`$show: SyncHook<Danmaku<T>>`**
- **`$pause: SyncHook<Danmaku<T>>`**
- **`$resume: SyncHook<Danmaku<T>>`**
- **`$destroy: SyncHook<Danmaku<T>>`**
- **`$moveEnd: SyncHook<Danmaku<T>>`**
- **`$moveStart: SyncHook<Danmaku<T>>`**
- **`$createNode: SyncHook<Danmaku<T>>`**
- **`$appendNode: SyncHook<Danmaku<T>>`**
- **`$removeNode: SyncHook<Danmaku<T>>`**
...
- **`unfreeze: SyncHook`**
- **`finished: SyncHook`**
- **`render: SyncHook<[DanmakuType]>`**
- **`mount: SyncHook<[HTMLElement]>`**
- **`unmount: SyncHook<HTMLElement | null>`**
- **`init: SyncHook<[manager: Manager<T>]>`**
- **`limitWarning: SyncHook<[DanmakuType, number]>`**
- **`updateOptions: SyncHook<[Partial<ManagerOptions>]>`**
- **`push: SyncHook<[PushData<T>, DanmakuType, boolean]>`**
- **`willRender: SyncWaterfallHook<{ prevent: boolean; type: DanmakuType; danmaku: Danmaku<T>}>`**
...
这里只会介绍 `manager` **自身的钩子**，因为弹幕钩子已经在前面的[章节](../hooks-danmaku)介绍过。
