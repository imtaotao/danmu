...
```ts
// 1. 弹幕的高度，如果你不需要通过计算就可以得到，则不需要通过 `getHeight()` 方法。
// 2. 确保获取的轨道存在，可以通过 `manager.trackCount` 来判断。
const { middle } = manager.getTrackLocation(0);
const y = middle - danmaku.getHeight() / 2;

danmaku.setStyle('top', `${y}px`);
```
