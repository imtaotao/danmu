...
`manager` 的 [**`options`**](../manager-config)，你可以从这里取到一些初始值并使用。
...
1. 高级弹幕**必须传递 `position` 来指定位置**。
2. 高级弹幕如果不传 `duration` 则会从 `manager.options.times` 范围里面**随机生成一个**。
3. 高级弹幕的插件和普通弹幕的插件一样，你可以通过 `danmaku.type` 属性来区分。
4. 高级弹幕如果不传 `direction` 则会取 **`manager.options.direaction`**。
