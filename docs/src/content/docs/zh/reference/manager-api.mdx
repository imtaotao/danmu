...
1. `number`: 默认单位为 `px`。
2. `string`: 表达式计算。支持（`+`, `-`, `*`, `/`, `%`）数学计算，暂时只支持 `%` 和 `px` 两种单位。
...
manager.pushFlexibleDanmaku(
  { content: '弹幕内容' },
  {
    ...
    position(danmaku, box) {
      return {
        x: (box.width - danmaku.getWidth()) * 0.5,
        y: (box.height - danmaku.getHeight()) * 0.5,
      };
    },
  },
);
...
**类型：`(container?: HTMLElement | string, { clear?: boolean }) => void`**
...
**类型：`(url?: string, el?: HTMLElement) => void`**
...
设置当前容器（`manager.box.node`）的大小，会自动 format，使用方法可以参见 [**demo**](https://github.com/imtaotao/danmu/blob/master/demo/src/components/sidebar/SidebarAreaY.tsx#L26-L31)。
