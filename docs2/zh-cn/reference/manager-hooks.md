...
这里只会介绍 `manager` **自身的钩子**，因为弹幕钩子已经在前面的[**章节**](../danmaku-hooks)介绍过。
...
**类型：`SyncWaterfallHook<{ prevent: boolean; type: DanmakuType; danmaku: Danmaku<T>}>`**

`willRender` 钩子会在弹幕即将要进入渲染队列的时候触发，你可以在此钩子阻止弹幕渲染，这对于通过关键字（或者其他条件）来过滤弹幕的功能实现很有用。
